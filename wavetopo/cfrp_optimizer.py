...
class MMA:
    def __init__(self, xmin, xmax, move=0.15, asyinit=0.2, asyincr=1.2,
                 ...
                 adapt=False, move_min=0.005, move_decr=0.55, move_incr=1.08):
        """adapt=False reproduces the original behaviour exactly (21 call sites
        depend on it).  With adapt=True and the objective value passed to
        update(), the move limit contracts after any step that WORSENED the
        objective and relaxes back otherwise.

        Why this matters: update() below returns the separable subproblem
        minimizer unconditionally -- there is no line search and no acceptance
        test -- so a step that overshoots the region where the MMA approximation
        is valid is accepted and persists until the asymptotes happen to
        contract.  In the time-harmonic cloak that shows up as deep transient
        dips in the convergence curve.  The objective is already evaluated every
        iteration, so using it to size the trust region costs nothing."""
        ...
        self.move_decr = move_decr
        self.move_incr = move_incr
        self._fprev = None

    def update(self, x, df0dx, f=None):
        """f: current objective value (the quantity being MINIMIZED).  Ignored
        unless adapt=True was set; supplying it then adapts the move limit."""
        x = np.asarray(x, float)
        if self.adapt and f is not None:
            if self._fprev is not None:
                if f > self._fprev:          # last step made things worse
                    self.move = max(self.move*self.move_decr, self.move_min)
                else:                        # relax back toward the initial cap
                    self.move = min(self.move*self.move_incr, self.move0)
            self._fprev = f
        self.it += 1
